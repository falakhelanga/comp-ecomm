...
const OrderScreen = ({ history }) => {
  ...
  const [stripeMess, setStripeMess] = useState("");
  ...
  useEffect(() => {
    ...
    const scriptCreator = async () => {
      ...
      const { data: CLIENT_ID } = await axios.get(
        "https://procomputers.herokuapp.com/api/clientId",
        config
      );
  ...
  useEffect(() => {
    if (stripered === "success") {
      setStripeMess("Order Placed Succefully");
    }

    if (stripered === "cancell") {
      setStripeMess("Order has Not Placed Please Try Again");
    }
  }, [stripered]);
  ...
  const stipePayMent = async () => {
    const { data } = await axios.get("http://localhost:500/api/stripeKey");
    const stripeKey = data;
  };
  return (
    <>
      {" "}
      {order && (
        <Container className="py-5">
          {stripered && (
            <Message variant={stripered === "success" ? "success" : "danger"}>
              {stripeMess}
            </Message>
          )}
          <h4
                        ...
                        <Col>
                          {order.isDeliverd ? (
                            <Message variant="success">
                              Delivered at{" "}
                              {new Date(order.delieveredAt).toDateString()}{" "}
                            </Message>
                          ) : (
